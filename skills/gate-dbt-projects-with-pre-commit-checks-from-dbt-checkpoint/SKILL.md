---
name: Gate dbt projects with pre-commit checks from dbt-checkpoint
description: Use dbt-checkpoint when an agent should catch dbt naming, dependency,
  metadata, and model-hygiene issues before review or merge.
category: Code Quality & Review
framework: Multi-Framework
verification: listed
source: https://github.com/dbt-checkpoint/dbt-checkpoint
tool_ecosystem:
  github_repo: dbt-checkpoint/dbt-checkpoint
  github_stars: 738
  tool: dbt-checkpoint
---
# Gate dbt projects with pre-commit checks from dbt-checkpoint
Use dbt-checkpoint when an agent should catch dbt naming, dependency, metadata, and model-hygiene issues before review or merge.

## Installation

### Option 1, Agent Skill Exchange

Browse and install from the marketplace page for this skill.

### Option 2, Git clone

```bash
git clone https://github.com/agentskillexchange/skills.git && cd skills/skills/gate-dbt-projects-with-pre-commit-checks-from-dbt-checkpoint
```

### Option 3, Download ZIP

Download the skill folder or repository archive and extract `skills/gate-dbt-projects-with-pre-commit-checks-from-dbt-checkpoint` into your local skills collection.

### Option 4, Manual copy

Copy this skill folder into your agent skills directory, then reload your agent tooling.

### Option 5, Fork and sync

Fork the repository if you want to track local edits while keeping a clean upstream sync path.

## Source

- [Agent Skill Exchange](https://agentskillexchange.com/skills/gate-dbt-projects-with-pre-commit-checks-from-dbt-checkpoint/)
