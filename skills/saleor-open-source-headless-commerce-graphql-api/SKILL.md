...
```bash
claude mcp add --scope user skills -- npx -y skills-mcp agentskillexchange/skills --skill saleor-open-source-headless-commerce-graphql-api
```
...
```json
{
  "mcpServers": {
    "skills": {
      "command": "npx",
      "args": ["-y", "skills-mcp", "agentskillexchange/skills", "--skill", "saleor-open-source-headless-commerce-graphql-api"]
    }
  }
}
```
...
```bash
npx skills add agentskillexchange/skills --skill saleor-open-source-headless-commerce-graphql-api --output ./skills
```
