...
```bash
claude mcp add --scope user skills -- npx -y skills-mcp agentskillexchange/skills --skill krakend-high-performance-stateless-api-gateway
```
...
```json
{
  "mcpServers": {
    "skills": {
      "command": "npx",
      "args": ["-y", "skills-mcp", "agentskillexchange/skills", "--skill", "krakend-high-performance-stateless-api-gateway"]
    }
  }
}
```
...
```bash
npx skills add agentskillexchange/skills --skill krakend-high-performance-stateless-api-gateway --output ./skills
```
