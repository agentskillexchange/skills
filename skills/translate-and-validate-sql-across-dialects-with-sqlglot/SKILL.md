...
description: "SQLGlot gives an agent a narrow, useful job: parse SQL, translate it across dialects, and catch syntax or semantic mismatches before a migration, review, or generated-query workflow ships bad SQL. Invoke it when the real task is cross-dialect validation or safe SQL rewriting, not when you just want a database engine to run the query. The boundary is specific and publishable: this is a SQL translation and validation workflow for agents, not a generic parser library card or a broad database platform listing."
verification: listed
