...
How It Works Logdy sits between your log source and the browser. Pipe any command output into Logdy (tail -f app.log | logdy) and it starts a local web server at port 8080. The embedded UI renders incoming log lines in real time with syntax highlighting, filtering, and search capabilities. It also supports reading files directly with logdy follow app.log --full-read, accepting data over sockets, and receiving logs via its REST API.
