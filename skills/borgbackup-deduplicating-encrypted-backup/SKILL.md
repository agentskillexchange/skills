...
Installation pip install borgbackup # Or use standalone binary from GitHub releases # Or via system package manager: apt install borgbackup Example Usage # Initialize a new encrypted repository borg repo-create -e repokey-aes-ocb /path/to/repo

# Create a backup borg create /path/to/repo::backup-{now} ~/Documents

# List archives borg list /path/to/repo

# Restore files borg extract /path/to/repo::backup-name

# Prune old backups (keep 7 daily, 4 weekly) borg prune --keep-daily 7 --keep-weekly 4 /path/to/repo
