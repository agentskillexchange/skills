---
title: Apache Superset Dashboard and SQL Exploration Skill
description: Apache Superset is a widely adopted open-source BI platform for SQL exploration,
  chart building, and dashboard delivery. This skill is useful when an agent needs
  to query warehouse data, assemble dashboards, or explain metrics using a mature
  analytics interface instead of ad hoc notebook code.
verification: security_reviewed
source: https://github.com/apache/superset
category:
- Data Extraction & Transformation
framework:
- Multi-Framework
tool_ecosystem:
  github_repo: apache/superset
  github_stars: 72339
---

# Apache Superset Dashboard and SQL Exploration Skill

Apache Superset is a widely adopted open-source BI platform for SQL exploration, chart building, and dashboard delivery. This skill is useful when an agent needs to query warehouse data, assemble dashboards, or explain metrics using a mature analytics interface instead of ad hoc notebook code.

## Installation

### Method 1, Agent Skill Exchange

- Install from the marketplace listing: https://agentskillexchange.com/skills/apache-superset-dashboard-sql-exploration-skill/

### Method 2, Git clone

```bash
git clone https://github.com/agentskillexchange/skills.git && cd skills/skills/apache-superset-dashboard-sql-exploration-skill
```

### Method 3, Download ZIP

- Download the repository ZIP and extract `skills/apache-superset-dashboard-sql-exploration-skill`.

### Method 4, Manual copy

- Copy this skill folder into your local skills directory, then reload your agent tooling.

### Method 5, Fork and sync

- Fork the repository if you want to maintain local edits while syncing upstream changes.

## Source

- [Agent Skill Exchange](https://agentskillexchange.com/skills/apache-superset-dashboard-sql-exploration-skill/)
