...
```bash
claude mcp add --scope user skills -- npx -y skills-mcp agentskillexchange/skills --skill formbricks-open-source-survey-experience-management
```
...
```json
{
  "mcpServers": {
    "skills": {
      "command": "npx",
      "args": ["-y", "skills-mcp", "agentskillexchange/skills", "--skill", "formbricks-open-source-survey-experience-management"]
    }
  }
}
```
...
```bash
npx skills add agentskillexchange/skills --skill formbricks-open-source-survey-experience-management --output ./skills
```
