...
xan is designed for Unix pipeline composition, reading from stdin and writing to stdout by default. It integrates naturally with other CLI utilities like jq, ripgrep, and standard Unix tools for end-to-end data processing workflows. Shell completions for bash, zsh, and fish are available via the xan completions command.
