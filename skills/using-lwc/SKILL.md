...
name: "Using LWC"
slug: "using-lwc"
description: "Preserves source-grounded project decisions, research, incident findings, and code context across agent sessions through LWC Wiki memory, physical document graphs, and CodeGraph indexes."
category: "Developer Tools"
framework: "Multi-Framework"
verification: listed
source: "https://github.com/JanYork/using-lwc"
tool_ecosystem:
  tool: "LWC"
  github_repo: "JanYork/llm-wiki-cli"
  npm_package: "@i-xor/lwc"
  license: "MIT"
  maintained: true
---
...
Install the canonical upstream skill with the pinned third-party `skills`
package version:

```bash
npm exec --package=skills@1.5.7 -- skills add JanYork/using-lwc --skill using-lwc
```

After this listing is merged, install the Agent Skill Exchange copy:

```bash
npm exec --package=skills@1.5.7 -- skills add agentskillexchange/skills --skill using-lwc
```

For manual installation, clone the Exchange repository, review the complete
bundle, and copy only this skill directory into the directory used by the agent:

```bash
git clone https://github.com/agentskillexchange/skills.git
mkdir -p ~/.agent-skills
cp -R skills/skills/using-lwc ~/.agent-skills/using-lwc
```
