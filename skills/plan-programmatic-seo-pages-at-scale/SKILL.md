---
title: Plan Programmatic SEO Pages at Scale
description: Design template and data driven SEO page programs that can scale without
  collapsing into thin, duplicate, or low-intent content.
verification: listed
source: https://github.com/coreyhaines31/marketingskills/tree/main/skills/programmatic-seo
category:
- Content Writing & SEO
framework:
- Multi-Framework
tool_ecosystem:
  github_repo: coreyhaines31/marketingskills
  github_stars: 21914
---

# Plan Programmatic SEO Pages at Scale

Design template and data driven SEO page programs that can scale without collapsing into thin, duplicate, or low-intent content.

## Installation

### Method 1, Agent Skill Exchange

- Install from the marketplace listing: https://agentskillexchange.com/skills/plan-programmatic-seo-pages-at-scale/

### Method 2, Git clone

```bash
git clone https://github.com/agentskillexchange/skills.git && cd skills/skills/plan-programmatic-seo-pages-at-scale
```

### Method 3, Download ZIP

- Download the repository ZIP and extract `skills/plan-programmatic-seo-pages-at-scale`.

### Method 4, Manual copy

- Copy this skill folder into your local skills directory, then reload your agent tooling.

### Method 5, Fork and sync

- Fork the repository if you want to maintain local edits while syncing upstream changes.

## Source

- [Agent Skill Exchange](https://agentskillexchange.com/skills/plan-programmatic-seo-pages-at-scale/)
