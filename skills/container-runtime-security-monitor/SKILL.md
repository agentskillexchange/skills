---
name: "Container Runtime Security Monitor"
description: "Monitors container runtime behavior using Falco rules and the Docker Engine API. Detects anomalous syscalls, privilege escalations, and unexpected network connections in real time."
category: "Security & Verification"
framework: "Gemini"
verification: "verified"
rating: "4.0"
reviews: "45"
creator: "Elena Rodriguez"
creator_handle: "@elena_dev"
creator_verified: true
source: "https://agentskillexchange.com/skill/container-runtime-security-monitor/"
---

# Container Runtime Security Monitor

Monitors container runtime behavior using Falco rules and the Docker Engine API. Detects anomalous syscalls, privilege escalations, and unexpected network connections in real time.

## Installation

### Any Agent (npx)

```bash
npx @anthropic/agent-skills install container-runtime-security-monitor
```

### Claude Code

```bash
claude mcp add container-runtime-security-monitor
```

### Cursor

Add to `.cursor/skills.json`:
```json
{
  "skills": ["container-runtime-security-monitor"]
}
```

### OpenClaw

```bash
clawhub install container-runtime-security-monitor
```

### Codex

```bash
codex install container-runtime-security-monitor
```

## Details

| Field | Value |
|-------|-------|
| **Category** | Security & Verification |
| **Framework** | Gemini |
| **Verification** | ✅ Verified |
| **Rating** | ⭐ 4.0 (45 reviews) |

## Creator

**Elena Rodriguez** ✅ Verified Creator
- Handle: @elena_dev
- [View Profile](https://agentskillexchange.com/creator/elena_dev)

## Links

- [View on Agent Skill Exchange](https://agentskillexchange.com/skill/container-runtime-security-monitor/)
- [Browse All Skills](https://agentskillexchange.com)
