...
What It Produces Hurl outputs response bodies, test results (pass/fail with detailed error messages), JUnit XML reports for CI integration, and HTML reports for human review. In test mode (--test), it provides a compact summary showing which assertions passed or failed. The --json flag outputs structured results for programmatic consumption.

Integration and Installation Hurl is available via Homebrew, Snap, Chocolatey, Scoop, Arch AUR, Conda, npm (@anthropic-ai/hurl), and as standalone binaries for Linux, macOS, and Windows. It integrates naturally with GitHub Actions, GitLab CI, Jenkins, and any CI system that can run shell commands. The hurlfmt companion tool formats and converts Hurl files to other formats.
