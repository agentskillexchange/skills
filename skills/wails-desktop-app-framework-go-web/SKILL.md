...
How It Works Wails binds Go functions directly to the JavaScript frontend. Developers write backend logic in Go and build the UI with React, Vue, Svelte, or plain HTML/CSS/JS. The wails build command compiles everything into a single native binary. No bundled Chromium, no Node.js runtime — just a lean executable.

Key Features

Native webview rendering: Uses the OS native webview instead of bundling Chromium, producing binaries as small as 10 MB compared to Electron 150 MB+. Go backend with JS frontend: Call Go functions from JavaScript and vice versa with automatic TypeScript bindings generation. Built-in templates: Scaffold projects with React, Vue, Svelte, Preact, Lit, or Vanilla JS templates. Native dialogs and menus: Access native file dialogs, system menus, and tray icons. Dark/Light mode: Automatic OS theme detection and switching. Live development mode: Hot reload for both Go and frontend code during development. Cross-platform: Build for Windows, macOS, and Linux from a single codebase.
...
Installation go install github.com/wailsapp/wails/v2/cmd/wails@latest
