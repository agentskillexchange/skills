---
name: "LightRAG Graph-Based Retrieval-Augmented Generation Framework"
description: "LightRAG is a Python-based retrieval-augmented generation framework that builds knowledge graphs from documents for more connected, contextual retrieval. Published at EMNLP 2025, it enables graph-powe"
category: "Data Extraction & Transformation"
framework: "Multi-Framework"
verification: "listed"
source: "https://github.com/HKUDS/LightRAG"
---

# LightRAG Graph-Based Retrieval-Augmented Generation Framework

LightRAG is a Python-based retrieval-augmented generation framework that builds knowledge graphs from documents for more connected, contextual retrieval. Published at EMNLP 2025, it enables graph-powered RAG with support for multiple storage backends and LLM providers.

## Installation

### Install via OpenClaw
```bash
openclaw skills install lightrag-graph-rag-framework
```

### Install via ClawHub CLI
```bash
clawhub install lightrag-graph-rag-framework
```

### Install via Smithery CLI
```bash
npx -y @anthropic-ai/sdk install lightrag-graph-rag-framework
```

### Manual Installation
Download the skill from the [Agent Skill Exchange](https://agentskillexchange.com/skills/lightrag-graph-rag-framework/) and follow the setup instructions.

## Source

- [Agent Skill Exchange](https://agentskillexchange.com/skills/lightrag-graph-rag-framework/)
- [Upstream Source](https://github.com/HKUDS/LightRAG)
