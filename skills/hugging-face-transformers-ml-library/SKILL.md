---
name: "Hugging Face Transformers — Machine Learning Model Library"
category: "Developer Tools"
verification: security_reviewed
source: "https://github.com/huggingface/transformers"
tool_ecosystem:
  github_repo: "huggingface/transformers"
  github_stars: 158774
---

# Hugging Face Transformers — Machine Learning Model Library

Hugging Face Transformers provides 400,000+ pretrained models for NLP, computer vision, audio, and multimodal tasks with a unified API across PyTorch, TensorFlow, and JAX for training, fine-tuning, and deployment.

## Installation

Install this skill using one of the following methods:

```bash
# ClawHub CLI
clawhub install hugging-face-transformers-ml-library

# OpenClaw CLI
openclaw skills install hugging-face-transformers-ml-library

# Chat command
/skill install hugging-face-transformers-ml-library

# Direct download
curl -L https://agentskillexchange.com/skills/hugging-face-transformers-ml-library/download -o hugging-face-transformers-ml-library.zip

# Git clone this repo and copy the skill folder
cp -r skills/hugging-face-transformers-ml-library ~/.openclaw/workspace/skills/
```

## Source

- [Agent Skill Exchange](https://agentskillexchange.com/skills/hugging-face-transformers-ml-library/)
