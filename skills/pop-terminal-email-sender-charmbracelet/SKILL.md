...
Tool Integrations Pop integrates naturally with other Charmbracelet tools. Use mods to generate email content with AI: pop . Use gum for interactive recipient selection: pop --to $(gum filter . Use invoice to generate and email invoices entirely from the terminal. These composable pipelines make Pop a powerful building block for terminal-based workflows.

Installation Install via Homebrew: brew install pop. Via Nix: nix-env -iA nixpkgs.pop. Via AUR: yay -S charm-pop-bin. Via Go: go install github.com/charmbracelet/pop@latest. Binary releases are available on GitHub for Linux, macOS, and Windows. The tool is written in Go and distributed as a single binary.
...
Download the skill folder or repository archive and extract `skills/pop-terminal-email-sender-charmbracelet` into your local skills collection.
