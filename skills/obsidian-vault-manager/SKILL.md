...
description: Obsidian Vault Manager is built around HashiCorp Vault secrets platform.
  ...
  thin one-line wrapper, using stable interfaces like KV v2, policies, leases, tokens,
  transit, dynamic secrets and preserving the operational context that matters for
  real tasks. In practice, the skill gives an agent a stable interface to vault so
  ...
  on KV v2, policies, leases, tokens, transit, dynamic secrets, with configuration
  passed through environment variables, connection strings, service tokens, or workspace
  config depending on the upstream platform. Accesses KV v2, policies, leases, tokens,
  transit, dynamic secrets instead of scraping a UI, which makes runs easier to audit
  ...
  integration points such as secret management, encryption, and runtime credential
  delivery. Key integration points include secret management, encryption, and runtime
  credential delivery. In a real environment that usually means passing credentials
