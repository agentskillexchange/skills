...
Core Capabilities The Typst CLI (typst compile and typst watch) takes .typ source files and produces PDF output. The system includes built-in markup for headings, emphasis, lists, and other common formatting tasks, eliminating the need for preambles or package imports that LaTeX requires. For advanced formatting, Typst provides set rules and show rules that let you configure or completely redefine how elements appear.

Scripting and Math Typst embeds a tightly integrated scripting language directly in documents. Variables, functions, loops, and conditionals work inline using hash-prefixed expressions. Math typesetting uses dollar-sign delimiters with a clean syntax — no backslashes needed for common symbols like sqrt, floor, or Greek letters. Bibliography management is built in, supporting BibTeX and other citation formats.
...
Download the skill folder or repository archive and extract `skills/typst-markup-typesetting-system` into your local skills collection.
