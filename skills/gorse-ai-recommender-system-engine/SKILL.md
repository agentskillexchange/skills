...
Getting Started The fastest way to try Gorse is via Docker: docker run -p 8088:8088 zhenghaoz/gorse-in-one --playground. This downloads sample data and starts the dashboard at http://localhost:8088. For production deployments, Gorse supports Kubernetes via Helm charts and can be configured for high availability with multiple server and worker nodes.
...
Download the skill folder or repository archive and extract `skills/gorse-ai-recommender-system-engine` into your local skills collection.
