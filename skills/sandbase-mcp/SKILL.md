...
SandBase provides access to 2,000+ AI models and API tools through a unified MCP interface. One account covers LLMs, image generation, video generation, audio, embeddings, web scraping, social media APIs, and more. This Skill is the official Agent Skill maintained and versioned with the open-source [sandbaseai/cli](https://github.com/sandbaseai/cli), giving MCP-compatible agents (Codex, Claude Code, Cursor, Gemini CLI, OpenClaw, Hermes) a disciplined `discover → inspect → run` workflow.

This is distinct from the cross-validate research entry: that entry is a focused multi-source research workflow from `sandbaseai/sandbase-skills`; this Skill is the general MCP integration Skill.
...
- [Agent Skill Exchange](https://agentskillexchange.com/skills/sandbase-mcp/)
- Upstream: https://github.com/sandbaseai/cli
