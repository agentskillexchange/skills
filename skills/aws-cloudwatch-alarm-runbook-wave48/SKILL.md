---
title: AWS CloudWatch Alarm Runbook
description: Automates incident response for AWS CloudWatch alarms using boto3, the
  CloudWatch GetMetricData API, and AWS Systems Manager runbook documents. Maps alarm
  states to diagnostic procedures and remediation actions.
verification: security_reviewed
source: https://github.com/aws/aws-sdk-js-v3
category:
- Runbooks & Diagnostics
framework:
- Claude Agents
tool_ecosystem:
  github_repo: aws/aws-sdk-js-v3
  github_stars: 3607
---

# AWS CloudWatch Alarm Runbook

Automates incident response for AWS CloudWatch alarms using boto3, the CloudWatch GetMetricData API, and AWS Systems Manager runbook documents. Maps alarm states to diagnostic procedures and remediation actions.

## Installation

### Method 1, Agent Skill Exchange

- Install from the marketplace listing: https://agentskillexchange.com/skills/aws-cloudwatch-alarm-runbook-wave48/

### Method 2, Git clone

```bash
git clone https://github.com/agentskillexchange/skills.git && cd skills/skills/aws-cloudwatch-alarm-runbook-wave48
```

### Method 3, Download ZIP

- Download the repository ZIP and extract `skills/aws-cloudwatch-alarm-runbook-wave48`.

### Method 4, Manual copy

- Copy this skill folder into your local skills directory, then reload your agent tooling.

### Method 5, Fork and sync

- Fork the repository if you want to maintain local edits while syncing upstream changes.

## Source

- [Agent Skill Exchange](https://agentskillexchange.com/skills/aws-cloudwatch-alarm-runbook-wave48/)
